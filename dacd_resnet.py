...
from imagenet_classification.models.utils import conv1x1, conv3x3, Masker_spatial, ExpandMask, apply_spatial_mask
...
class Cacd_Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, group_width=1,
                 ...
                 output_size=np.array([100,3]),  # 输出尺寸
                 mask_spatial_granularity=np.array([5,1])):
        ...
        self.mask_size = np.array([self.output_size[0] // self.mask_spatial_granularity[0], self.output_size[1] // self.mask_spatial_granularity[1]])
...
class Cacd_ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=6, zero_init_residual=False,
                 ...
                 norm_layer=None,
                 input_size=[(200,3),(100,2),(50,1)],  # 输入尺寸
                 mask_spatial_granularity=[(5,1),(5,1),(5,1)],    # 像素级别  
                 lr_mult=1.0,
                 **kwargs):
